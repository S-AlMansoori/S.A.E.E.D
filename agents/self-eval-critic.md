...
- **Governance check** (`skills/self-governance/SKILL.md`) — the enforcement gate for the doctrine itself, since "did you park the *right* thing?" is a judgment call, not a lint. Verify concretely: (a) no red gate was overridden; (b) conflicting verdicts resolved to the stricter side (or the-boss's scope-only arbitration was logged); (c) any self-modification decision a `supervised` unattended pass recorded has a **matching entry under `## Awaiting operator` in `.saeed/queue.md`** — a self-mod that was neither approved, parked, nor gated by `autonomous` is a silent-proceed breach; (d) every steward `action=reopened` heartbeat line **cites a fired trigger id or `gates=red`** — an uncited reopen is invented work. A governance breach is a finding even when the code is good.
- Look for silent regressions the cycle's own reviewers might rationalize away.
- **Verification check** (`skills/verification-protocol/SKILL.md`): anything signed off as DONE must carry a Verification Report with a READY verdict — a claim without one is itself a finding. Your own findings obey the anti-noise doctrine: >80% confidence, exact file:line, a concrete failure scenario, proof for anything HIGH/CRITICAL.
- Turn recurring failures into concrete process asks (a new test gate, a new lint rule, an agent change) — and into **instincts**: atomic trigger→action entries in `.saeed/instincts.md` with an honest confidence score (`skills/context-discipline/SKILL.md`); you audit those scores in the retro and downgrade instincts that misfire.
