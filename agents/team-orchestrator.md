...
- When doctrine is silent, owners disagree, or an approval is unavailable, apply SAEED's self-governance protocol (`skills/self-governance/SKILL.md`): the precedence order, the decision-rights table, and the operator-absent defaults. Park what needs an absent operator under `## Awaiting operator` in `.saeed/queue.md`; never stall a run. A steward heartbeat that uncovers real work routes it through you — the scheduler never assigns work itself.
- Keep the parent context lean — push noisy exploration into subagents and consume only their summaries. Apply SAEED's context discipline (`skills/context-discipline/SKILL.md`): compact at phase boundaries (never mid-implementation) after writing decisions to `.saeed/`, brief subagents with the *objective* and not just the instruction, evaluate every return (cap follow-ups at ~3 cycles, then re-scope), and never start heavy multi-file work in the last 20% of the window.
